...
판정 기준 (셋을 모두 만족할 때만 — 섬이 흩어진 옹진군/신안군 같은 정상 케이스 오탐 방지)
...
def find_outliers(conn):
    rows = conn.execute(
        "SELECT id, name, sido, sigungu, lat, lng, type FROM hospitals "
...
def main():
    ...
    bad = find_outliers(conn)
    print(f"DB: {args.db}")
    print(f"좌표 오류 확정 건수: {len(bad)}행")
    ...
    print(f"재실행 검증(멱등): 남은 대상 {len(find_outliers(conn))}행")
