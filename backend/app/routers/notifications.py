...
async def list_notifications(db: AsyncSession = Depends(get_db), user: User = Depends(require_user)):
    ...
    for n in notices:
        items.append({"type": "notice", "label": "병원용 공지사항", "title": n.title, "created_at": n.created_at, "link": "/notices/hospital"})

    news = (await db.execute(select(NewsArticle).order_by(NewsArticle.created_at.desc()).limit(10))).scalars().all()
    for a in news:
        items.append({"type": "news", "label": "의료소식", "title": a.title, "created_at": a.created_at, "link": "/news"})
    ...
    for p in posts:
        ...
        items.append({"type": "tech-post", "label": p.category, "title": p.title, "created_at": p.created_at, "link": link})
    ...
    if user.hospital_profile_id:
        ...
        for c, tid in rows:
            items.append({"type": "cs", "label": "CS 답변", "title": "내 CS 문의에 답변이 등록되었습니다", "created_at": c.created_at, "link": f"/cs/{tid}"})
