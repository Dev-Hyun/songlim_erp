...
export default function AdminPage() {
  ...
  if (authLoading || loading) return <div className="empty-state">불러오는 중...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="surface-card p-3">
        <h2 className="mb-3 card-title">회원가입 승인 대기 ({pendingUsers.length})</h2>
      ...
      <div className="surface-card p-3">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h2 className="card-title">사용자 목록 ({visibleUsers.length})</h2>
          <input
            value={userSearch}
            onChange={(e) => setUserSearch(e.target.value)}
            placeholder="🔍 아이디/이름/부서·직급/병원명/연락처로 검색"
            className="w-full rounded-control border border-gray-300 px-3 py-1.5 text-xs dark:border-gray-700 dark:bg-gray-900 sm:w-80"
          />
        </div>
        <div className="overflow-x-auto">
        ...
        {users.length > 0 && visibleUsers.length === 0 && (
          <div className="empty-state">&quot;{userSearch}&quot; 검색 결과가 없습니다</div>
        )}
        </div>
      </div>
