  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatTile label="등록 대수" value={detail.stats.units} sub={`${detail.name} · ${detail.code}`} />
        <StatTile
          label="분류 내 순위"
          value={`${detail.stats.rank}위`}
          sub={`${detail.stats.models_in_category.toLocaleString()}종 중 · 분류 기관의 ${detail.stats.share.toFixed(1)}%`}
        />
        <StatTile
          label="가장 많은 종별"
      ...
      >
        {list && list.items.length > 0 ? (
          <>
            ...
            <Pagination page={page} pageSize={PAGE_SIZE} total={list.total} onChange={setPage} />
          </>
        ) : (
          <EmptyState message="보유 의료기관이 없습니다." />
        )}
      </Panel>
      ...
      {detail.peers.length > 0 && (
        <Panel title="같은 분류의 상위 모델" desc="순위·추천이 아니라 확인 기관 수 정렬입니다">
          <div>
            {detail.peers.map((p) => (
