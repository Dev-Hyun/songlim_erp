...
function Card({
  ...
  return (
    <div
      ref={ref}
      onClick={onOpen}
      className={`relative cursor-pointer rounded-card border border-gray-200 bg-white p-3.5 transition hover:-translate-y-0.5 hover:border-brand-300 dark:border-gray-800 dark:bg-white/[0.03] ${
    ...
    >
      ...
      <div className="mb-3 flex h-20 items-center justify-center overflow-hidden rounded-control bg-brand-50 text-2xl dark:bg-brand-500/10">
        {item.image_key ? (
      ...
      <div className="mb-1 flex flex-wrap items-center gap-1.5">
        <span className="rounded-full bg-brand-50 px-1.5 py-0.5 text-[10px] font-medium text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">
          {item.category}{item.sub_category ? ` · ${item.sub_category}` : ""}
        </span>
        {item.code && <span className="text-[10px] text-gray-400">#{item.code}</span>}
      </div>
      <div className="mb-1.5 line-clamp-2 min-h-[34px] text-ui font-semibold leading-snug text-gray-800 dark:text-white/90">{item.name}</div>
      <dl className="mb-2 space-y-0.5 text-ui-xs">
      ...
      <div className="text-base font-semibold text-gray-900 dark:text-white">{item.unit_price.toLocaleString()}원</div>
      {!item.is_active && <span className="mt-1.5 inline-block rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-medium text-gray-500 dark:bg-white/10">비노출</span>}
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex flex-col bg-white dark:bg-gray-900">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-gray-200 px-5 py-3.5 dark:border-gray-800">
        <h2 className="shrink-0 card-title">소모품 품목 관리자 사이트</h2>
        <div className="relative min-w-0 flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full rounded-control border border-gray-300 bg-white py-2 pl-9 pr-3 text-xs dark:border-gray-700 dark:bg-gray-900"
          />
        </div>
        <button onClick={onClose} className="shrink-0 rounded-full border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-600 dark:border-gray-700 dark:text-gray-300">
          닫기
        </button>
      </div>
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <DndProvider backend={HTML5Backend}>
          <div className="flex-1 overflow-y-auto p-5">
            <div className="seg mb-2 flex-wrap">
              <button
                onClick={() => selectCategory(null)}
                className={`seg-item ${!categoryFilter ? "seg-item-on" : ""}`}
              >
                전체
              </button>
              {categories.map((c) => (
                <button
                  ...
                  className={`seg-item ${categoryFilter === c ? "seg-item-on" : ""}`}
                >
                  {c}
                </button>
              ))}
              {q && <span className="ml-1 fg-subtle text-ui-xs">&quot;{search}&quot; 검색 결과 {displayItems.length}건</span>}
            </div>
            {categoryFilter && subCategories.length > 0 && (
              <div className="mb-4 flex flex-wrap items-center gap-1.5">
                <button
                  onClick={() => setSubCategoryFilter(null)}
                  className={`rounded-full px-2.5 py-1 text-ui-xs font-semibold ${!subCategoryFilter ? "bg-brand-100 text-brand-700 dark:bg-brand-500/20 dark:text-brand-300" : "bg-gray-50 text-gray-500 dark:bg-white/5"}`}
                >
                  소분류 전체
                </button>
                {subCategories.map((sc) => (
                  <button
                    ...
                    className={`rounded-full px-2.5 py-1 text-ui-xs font-semibold ${subCategoryFilter === sc ? "bg-brand-100 text-brand-700 dark:bg-brand-500/20 dark:text-brand-300" : "bg-gray-50 text-gray-500 dark:bg-white/5"}`}
                  >
                    {sc}
                  </button>
                ))}
            ...
            {canDrag && <div className="mb-3 fg-subtle text-ui-xs">드래그해서 이 카테고리 안에서 순서 변경</div>}
            {loading ? (
              ...
              <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                <button
                  onClick={openNew}
                  className={`flex h-full min-h-[190px] flex-col items-center justify-center gap-2 rounded-card border-2 border-dashed p-3.5 text-ui font-medium ${
        ...
        <div className="w-full shrink-0 space-y-3 overflow-y-auto border-t border-gray-200 p-5 dark:border-gray-800 lg:w-80 lg:border-l lg:border-t-0">
          <h3 className="text-ui font-medium text-gray-800 dark:text-white/90">{editing ? `수정: ${editing.name}` : "새 품목 추가"}</h3>
          <button
            onClick={() => imageInputRef.current?.click()}
            className="flex h-24 w-full items-center justify-center overflow-hidden rounded-card border border-dashed border-gray-300 bg-gray-50 fg-subtle text-ui-sm dark:border-gray-700 dark:bg-white/5"
          >
            {form.image_key ? (
              // eslint-disable-next-line @next/next/no-img-element
          ...
          <p className="text-ui-xs leading-relaxed text-gray-400">
            권장 사진 규격: 정사각형 1000×1000px 이상 (예: 휴대폰 카메라로 품목을 가운데 두고 촬영 후 정사각형으로 자르기).
            모바일·PC 카드에서 모두 잘리지 않고 선명하게 보입니다.
          </p>

          <input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="코드" className="field-auto w-full" />
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="품목명" className="field-auto w-full" />
          <input value={form.manufacturer} onChange={(e) => setForm({ ...form, manufacturer: e.target.value })} placeholder="제조사" className="field-auto w-full" />
          <input value={form.spec} onChange={(e) => setForm({ ...form, spec: e.target.value })} placeholder="규격(선택)" className="field-auto w-full" />
          <div>
            <select
            ...
            {!categories.includes(form.category) && (
              <input
                ...
                placeholder="새 대분류명"
                className="mt-1.5 field-auto w-full"
              />
            )}
          </div>
          <div>
          ...
          <input value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} placeholder="단위 (예: 100입/1box)" className="field-auto w-full" />
          <input type="number" value={form.unit_price} onChange={(e) => setForm({ ...form, unit_price: Number(e.target.value) })} placeholder="기본금액" className="field-auto w-full" />
          <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="설명(선택)" rows={2} className="field-auto w-full" />
          <label className="flex items-center gap-2 text-ui text-gray-600 dark:text-gray-300">
            <input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} />
            병원측에 노출
          </label>

          <div className="flex gap-2 pt-1">
            <button onClick={handleSave} disabled={saving} className="flex-1 rounded-control bg-brand-500 px-4 py-2.5 text-ui font-medium text-white disabled:opacity-50">
              {editing ? "수정 저장" : "추가"}
            </button>
            {editing && (
              <button onClick={() => handleDelete(editing.id)} className="rounded-control border border-error-300 px-4 py-2.5 text-ui font-medium text-error-500 dark:border-error-800">
                삭제
              </button>
            )}
          </div>
