    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {(ticket.is_mine || user?.is_admin) && (
          <button onClick={deleteTicket} className="text-ui-sm font-medium text-error-600 hover:underline dark:text-error-400">
            삭제
          </button>
        )}
      </div>
      ...
      <div className="surface-card p-3">
        <div className="mb-2 text-xs font-medium uppercase text-gray-400">💬 답변/댓글 {comments.length}</div>
        <div className="mb-3 space-y-2">
          {comments.map((c) => (
            <div key={c.id} className="rounded-control bg-gray-50 p-2.5 text-ui dark:bg-white/[0.02]">
              <div className="mb-1 fg-subtle text-ui-xs">{c.created_at?.slice(0, 16).replace("T", " ")}</div>
              <div
          ...
          {comments.length === 0 && <div className="fg-subtle text-ui-sm">아직 답변이 없습니다</div>}
        </div>
        {isStaff ? (
          <div className="space-y-2">
            <RichTextEditor key={commentEditorKey} value={commentText} onChange={setCommentText} placeholder="답변을 입력하세요" minHeight="80px" />
            <button onClick={submitComment} className="rounded-control bg-brand-500 px-4 py-1.5 text-ui font-medium text-white">등록</button>
          </div>
        ) : (
          <div className="text-center fg-subtle text-ui-sm">답변은 송림 담당자가 등록합니다</div>
        )}
      </div>
    </div>
  );
